...
tags: ["query:rl-vg"]
score: 10.0
evidence: 面向几何的视频扩散奖励模型
tldr: 针对视频扩散模型缺乏几何监督导致的变形和漂移问题，本文提出VIGOR，一种基于几何的奖励模型。该模型利用预训练几何基础模型，通过跨帧重投影误差评估多视图一致性，并引入几何感知采样策略以提高鲁棒性。通过后训练对齐和推理侧优化，显著提升了视频生成的几何一致性，为增强开源模型提供了高效且实用的方案。
