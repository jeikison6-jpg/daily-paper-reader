...
tags: ["query:rl-vg"]
score: 10.0
evidence: 视频扩散模型中用于时间一致性的几何奖励模型
tldr: 针对视频扩散模型缺乏几何监督导致的变形和漂移问题，本文提出VIGOR，一种基于几何的奖励模型。该模型利用预训练几何基础模型，通过跨帧重投影误差评估多视图一致性，并引入几何感知采样策略。通过后训练对齐和推理侧优化，显著提升了视频生成的几何一致性，为增强开源模型提供了高效方案。
...
result: 实验证明该几何奖励模型比现有指标更具鲁棒性，能有效减少生成视频中的几何伪影。
