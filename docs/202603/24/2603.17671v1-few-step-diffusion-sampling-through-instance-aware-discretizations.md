...
tldr: 本研究针对扩散模型和流匹配模型在采样过程中使用全局统一离散化步长导致效率受限的问题，提出了一种实例感知（Instance-Aware）的离散化框架。该方法通过学习根据输入先验动态调整时间步分配，将基于梯度的离散化搜索扩展到条件生成场景。实验证明，该方法在图像和视频生成任务中，仅需极小的推理开销即可显著提升生成质量。
selection_source: fresh_fetch
motivation: 现有的扩散模型采样通常采用全局共享的时间步调度，忽略了不同样本生成过程中的复杂性差异，导致采样效率和质量并非最优。
method: 提出一种实例感知的离散化框架，利用输入相关的先验知识动态调整时间步分配，实现针对特定样本的自适应离散化搜索。
result: 在合成数据、像素空间扩散、潜空间图像及视频流匹配模型等多种设置下，该方法均能以极低的推理开销显著提升生成质量。
conclusion: 实例感知的离散化策略是优化扩散模型少步采样的有效途径，为提升生成模型的效率和灵活性提供了新方向。
---
