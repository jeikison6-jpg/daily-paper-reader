...
tags: ["query:rl-dm"]
score: 10.0
evidence: 将反向扩散采样视为MDP中的随机控制
tldr: 本研究提出了Diffusion Controller (DiffCon)框架，从控制论视角将扩散模型的反向采样统一为线性可解马尔可夫决策过程（LS-MDPs）中的随机控制问题。通过重加权预训练转移核，该框架推导出基于f-散度正则化的强化学习微调方法，并提出一种轻量级侧边网络参数化方案。实验证明，DiffCon在Stable Diffusion v1.4上的偏好对齐胜率和质量效率权衡均优于LoRA等基准。
selection_source: fresh_fetch
motivation: 旨在解决可控扩散生成中启发式方法缺乏统一理论理解的问题，建立扩散采样与随机控制之间的联系。
method: 将反向扩散建模为LS-MDPs，推导出f-散度正则化的策略梯度和奖励加权回归算法，并采用冻结主体的侧边网络进行参数化。
result: 在Stable Diffusion v1.4的监督和奖励驱动微调实验中，该方法在人类偏好胜率和生成效率上均显著优于LoRA等现有技术。
conclusion: DiffCon为扩散模型的可控生成提供了坚实的理论基础和高效的微调算法，实现了性能与参数效率的平衡。
---

## 摘要
可控扩散生成通常依赖于各种启发式方法，这些方法在缺乏统一理解的情况下似乎是相互孤立的。我们通过扩散控制器（DiffCon）弥补了这一差距，这是一种统一的控制论视角，将反向扩散采样建模为（广义）线性可解马尔可夫决策过程（LS-MDPs）中的纯状态随机控制。在该框架下，控制通过对预训练的反向时间转移核进行重加权来发挥作用，从而在终端目标与 $f$-散度代价之间取得平衡。根据所得的最优性条件，我们推导出了用于扩散微调的实用强化学习方法：(i) $f$-散度正则化的策略梯度更新，包括一种 PPO 风格的规则；(ii) 一种由正则化项决定的奖励加权回归目标，在 Kullback-Leibler (KL) 散度下具有极小值保持保证。LS-MDP 框架进一步暗示了一种有原则的模型形式：最优分数可分解为固定的预训练基准加上轻量级的控制修正，这启发了以暴露的中间去噪输出为条件的侧边网络参数化，从而在冻结主干网络的情况下实现有效的灰盒适配。在 Stable Diffusion v1.4 上进行的监督式和奖励驱动微调实验表明，与灰盒基准甚至参数高效的白盒适配器 LoRA 相比，该方法在偏好对齐胜率方面取得了持续提升，并改善了质量与效率之间的权衡。
