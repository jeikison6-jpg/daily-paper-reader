...
score: 8.0
evidence: 多智能体强化学习架构
tldr: 针对协作式多智能体强化学习中局部与全局视角难以平衡的问题，本文提出了一种分层引导评论家（HLC）架构。该方法受团队组织结构启发，通过多层级序列化训练方案，将高层目标设定与底层执行相结合。实验证明，HLC在部分可观测的协作任务中表现优异，具有高样本效率和强鲁棒性，且能有效扩展至多智能体规模。
selection_source: fresh_fetch
motivation: 传统的协作式多智能体强化学习往往局限于单一的局部或全局视角，难以在复杂任务中实现高效协调。
method: 提出分层引导评论家（HLC）架构，通过模拟团队层级结构，在不同层级上结合局部与全局视角进行序列化训练。
result: 在多个协作、非通信及部分可观测的基准测试中，HLC的性能优于单层级基准模型，并展现出良好的扩展性。
conclusion: 引入多层级架构并融合多维视角能显著提升多智能体系统的学习效率和策略鲁棒性。
---
