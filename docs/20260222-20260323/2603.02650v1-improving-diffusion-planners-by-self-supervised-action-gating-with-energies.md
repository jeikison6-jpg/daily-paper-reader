...
tags: ["query:rl-dm"]
score: 10.0
evidence: 通过动作门控改进离线强化学习中的扩散规划器
tldr: 针对扩散规划器在离线强化学习中因价值引导导致轨迹动力学不一致的问题，本文提出SAGE方法。SAGE通过自监督学习JEPA编码器和潜空间预测器，在推理阶段利用潜空间预测误差作为能量评分，对候选轨迹进行重排序。该方法无需环境交互或重新训练策略，显著提升了扩散规划器在多种任务中的性能和鲁棒性。
selection_source: fresh_fetch
motivation: 扩散规划器在推理时容易选择高价值但动力学不一致的轨迹，导致执行失败。
method: 提出SAGE方法，利用JEPA架构在潜空间评估轨迹的动力学一致性，并结合价值估计进行动作选择。
result: 在运动、导航和操作等多个基准测试中，SAGE显著提高了现有扩散规划器的性能和鲁棒性。
conclusion: SAGE作为一种即插即用的推理增强方法，能有效解决扩散规划中的可行性问题，且无需额外的环境模拟或模型重训。
---

## 摘要
扩散规划器是离线强化学习的一种强大方法，但当价值引导的选择倾向于得分高但在局部与环境动力学不一致的轨迹时，它们可能会失效，从而导致执行的脆弱性。我们提出了基于能量的自监督动作门控（SAGE），这是一种推理阶段的重排序方法，利用潜在一致性信号来惩罚动力学不一致的规划。SAGE 在离线状态序列上训练联合嵌入预测架构（JEPA）编码器，并针对短时域转换训练动作条件潜在预测器。在测试时，SAGE 根据潜在预测误差为每个采样的候选轨迹分配一个能量值，并将该可行性评分与价值估计相结合来选择动作。SAGE 可以集成到现有的扩散规划流水线中，这些流水线能够采样轨迹并通过价值评分选择动作；它不需要环境回放，也不需要重新训练策略。在运动、导航和操作基准测试中，SAGE 提高了扩散规划器的性能和鲁棒性。
