...
tags: ["query:rl-dm"]
score: 9.0
evidence: 基于可达性流策略的离线安全强化学习
tldr: 针对离线安全强化学习中实时控制与严格安全约束的挑战，本文提出 Safe Flow Q-Learning (SafeFQL)。该方法结合了 Hamilton-Jacobi 可达性启发的安全价值函数与高效的一步流策略，通过自洽贝尔曼递归学习安全边界，并利用共形预测进行校准。SafeFQL 在保持高性能的同时显著降低了推理延迟和约束违反率，为实时安全关键任务提供了强有力的支持。
selection_source: fresh_fetch
motivation: 现有的离线安全强化学习方法在处理实时控制任务时，往往面临推理延迟高或安全约束不够严格的问题。
method: 结合 Hamilton-Jacobi 可达性理论与流匹配技术，通过自洽贝尔曼递归学习安全价值，并将其蒸馏为高效的一步执行策略，辅以共形预测校准安全阈值。
result: 在多项基准测试中，SafeFQL 在推理速度上显著优于扩散模型类方法，同时在奖励最大化和减少安全违规方面达到或超过了现有最优水平。
conclusion: SafeFQL 证明了通过结合可达性分析与流策略蒸馏，可以在离线强化学习中实现兼顾实时性、高性能与严格安全保障的目标。
---
